...
public class SmoothProgressDrawable extends Drawable implements Animatable {
	...
	private SmoothProgressDrawable(Interpolator interpolator, int sectionsCount, int separatorLength, int[] colors,
			float strokeWidth, float speed, float progressiveStartSpeed, float progressiveStopSpeed, boolean reversed,
			boolean mirrorMode, Callbacks callbacks, boolean progressiveStartActivated, Drawable backgroundDrawable,
	...
	private void drawGradient(Canvas canvas) {
		...
		LinearGradient linearGradient = new LinearGradient(left, top, right, bottom, mLinearGradientColors,
	...
	private void drawStrokes(Canvas canvas) {
		...
		for (int i = 0; i <= mCurrentSections; ++i) {
			...
			ratioSectionWidth = Math.abs(mInterpolator.getInterpolation(prev) - mInterpolator.getInterpolation(Math.min(
					xOffset, 1f)));
	...
	private void drawLine(Canvas canvas, int canvasWidth, float startX, float startY, float stopX, float stopY,
	...
	public void setPosition(float pos) {
		mCurrentOffset = (int) (pos * mMaxOffset / 100.0);
		invalidateSelf();
	}
	...
	public static class Builder {
		...
		public SmoothProgressDrawable build() {
			...
			SmoothProgressDrawable ret = new SmoothProgressDrawable(mInterpolator, mSectionsCount,
					mStrokeSeparatorLength, mColors, mStrokeWidth, mSpeed, mProgressiveStartSpeed,
					mProgressiveStopSpeed, mReversed, mMirrorMode, mOnProgressiveStopEndedListener,
					mProgressiveStartActivated, mBackgroundDrawableWhenHidden, mGradients);
			return ret;
		}
