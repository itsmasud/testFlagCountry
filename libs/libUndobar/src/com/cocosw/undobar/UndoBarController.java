...
public class UndoBarController extends LinearLayout {
	...
	public static final UndoBarStyle UNDOSTYLE = new UndoBarStyle(drawable.ic_undobar_undo, string.undo);
	private UndoBarStyle style = UndoBarController.UNDOSTYLE;
	public static final UndoBarStyle RETRYSTYLE = new UndoBarStyle(drawable.ic_retry, string.retry, -1);
	...
	public static UndoBarController show(final Activity activity, final CharSequence message,
			final UndoListener listener, final Parcelable undoToken, final boolean immediate, final UndoBarStyle style) {
		return show(activity, message, listener, undoToken, immediate, style, -1);

	}

	public static UndoBarController show(final Activity activity, final CharSequence message, UndoListener listener,
			Parcelable undoToken, boolean immediate, UndoBarStyle style, int translucent) {
	...
	public static UndoBarController show(final Activity activity, final int message, final UndoListener listener,
		...
		return UndoBarController.show(activity, activity.getText(message), listener, undoToken, immediate,
				UndoBarController.UNDOSTYLE);
	}

	public static UndoBarController show(final Activity activity, final CharSequence message,
			final UndoListener listener, final Parcelable undoToken) {
		return UndoBarController.show(activity, message, listener, undoToken, false, UndoBarController.UNDOSTYLE);
	}

	public static UndoBarController show(final Activity activity, final CharSequence message,
			final UndoListener listener, final UndoBarStyle style) {
	 ...
	 * 
	 * @param activity
	 *            The activity where the undobar in
	 */
	...
	private void showUndoBar(final boolean immediate, final CharSequence message, final Parcelable undoToken) {
		...
		if (style.titleRes > 0) {
			...
			if (style.iconRes > 0) {
				mButton.setCompoundDrawablesWithIntrinsicBounds(getResources().getDrawable(style.iconRes), null, null,
						null);
			}
		} else {
	...
	public interface UndoListener {
		...
		/**
		 * The callback function will be called when user press button in
		 * Undobar
		 * 
		 * @param token
		 */
	...
	public interface AdvancedUndoListener extends UndoListener {
		/**
		 * The callback function will be called when the Undobar fade out after
		 * duration without button clicked.
		 * 
		 * @param token
		 */
