...
//    }
    ...
    public boolean isNextTokenNull() {
        return _source.regionMatches(_index, "null", 0, 4);
    }
    ...
    protected Object parseValue() throws ParseException {
        ...
        throw new ParseException("Could not detect object type! (" + _source
                + ")", _source.length() - _source.length());
    }
    ...
    private String parseString() {
        ...
        if (isAlphaNumeric(c)) {
            i = 1;
            key.append(c);
            while (isAlphaNumeric((c = _source.charAt(i + _index)))) {
                key.append(c);
                i++;
            }
            ...
            i = 1;
            while ((c = _source.charAt(i + _index)) != '"') {
                if (c == '\\') {
                    i++;
                    c = _source.charAt(i + _index);
                    switch (c) {
                        case '"':
                            key.append("\"");
                            break;
